...
/*   Updated: 2021/03/11 22:34:45 by dwinky           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int			ft_get_textures(void *mlx, t_texture *texture, t_config *config)
{
	...
	texture[0].addr = mlx_get_data_addr(texture[0].img, &texture[0].b_p_p, &texture[0].line_len, &texture[0].endian);
	if (texture[0].addr == NULL)
		return (1);
	texture[1].img = mlx_xpm_file_to_image(mlx, config->so, &texture[1].width, &texture[1].height);
	texture[1].addr = mlx_get_data_addr(texture[1].img, &texture[1].b_p_p, &texture[1].line_len, &texture[1].endian);
	if (texture[1].addr == NULL)
		return (1);
	texture[2].img = mlx_xpm_file_to_image(mlx, config->we, &texture[2].width, &texture[2].height);
	texture[2].addr = mlx_get_data_addr(texture[2].img, &texture[2].b_p_p, &texture[2].line_len, &texture[2].endian);
	if (texture[2].addr == NULL)
		return (1);
	texture[3].img = mlx_xpm_file_to_image(mlx, config->ea, &texture[3].width, &texture[3].height);
	texture[3].addr = mlx_get_data_addr(texture[3].img, &texture[3].b_p_p, &texture[3].line_len, &texture[3].endian);
	if (texture[3].addr == NULL)
		return (1);
	texture[4].img = mlx_xpm_file_to_image(mlx, config->s, &texture[4].width, &texture[4].height);
	texture[4].addr = mlx_get_data_addr(texture[4].img, &texture[4].b_p_p, &texture[4].line_len, &texture[4].endian);
